...
class DeepLinkBuilder:
    ...
    def chatroom(workflow_id: str, agent: str) -> str:
        ...
        The resulting URL carries the intent so the AOS can pre-warm the
        context before the founder even describes their problem.
        """
...
class JsonLDGenerator:
    ...
    def faq(category: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Each panic search query becomes a question, and its legendary
        diagnostic becomes the answer.
        """
        main_entity = []
...
def total_query_count() -> int:
    """Return the total number of panic search queries across all categories."""
    return PainTaxonomy.total_query_count()
...
def get_category_by_slug(slug: str) -> Dict[str, Any]:
    ...
    Raises:
        KeyError: If the slug is not found.
    """
    return PainTaxonomy.get_by_slug(slug)
...
def get_category_by_number(number: int) -> Dict[str, Any]:
    ...
    Raises:
        KeyError: If the number is not found.
    """
    return PainTaxonomy.get_by_number(number)
...
def chatroom_deep_link(workflow_id: str, agent: str) -> str:
    """Build a parametric chatroom URL that pre-loads a specific workflow."""
    return DeepLinkBuilder.chatroom(workflow_id, agent)


def github_yaml_url(yaml_path: str) -> str:
    """Build a GitHub URL for viewing the source YAML spec."""
    return DeepLinkBuilder.github_yaml(yaml_path)


def spec_page_url(workflow_id: str) -> str:
    """Build a website URL for a workflow's public spec page."""
    return DeepLinkBuilder.spec_page(workflow_id)


def pillar_page_url(category_slug: str) -> str:
    """Build a website URL for a pain category's pillar page."""
    return DeepLinkBuilder.pillar_page(category_slug)
...
def generate_faq_jsonld(category: Dict[str, Any]) -> Dict[str, Any]:
    """Generate JSON-LD FAQPage schema for a single pain category."""
    return JsonLDGenerator.faq(category)


def generate_knowledge_graph_jsonld() -> Dict[str, Any]:
    """Generate a comprehensive JSON-LD Knowledge Graph for the entire site."""
    return JsonLDGenerator.knowledge_graph()
...
def generate_workflow_spec_jsonld(
    ...
    """Generate JSON-LD SoftwareSourceCode schema for a workflow spec page."""
    return JsonLDGenerator.workflow_spec(category)


def render_jsonld_script(data: Dict[str, Any]) -> str:
    """Render a JSON-LD dict as an HTML ``<script>`` tag."""
    return JsonLDGenerator.render_script(data)
