...
This module defines the canonical SEO taxonomy that maps 10 pain categories
to their legendary CXO owners, panic-driven search queries (with legendary
diagnostics), workflow CTAs, and JSON-LD structured data.

The taxonomy is the single source of truth for generating:
- Pillar pages (one per pain category)
- The Entropy Index hub page
- Workflow "Public Spec" pages
- JSON-LD FAQ Schema (Featured Snippet targeting)
- JSON-LD Knowledge Graph (site-wide semantic map)
- Parametric deep-links to the chatroom
...
def get_category_by_slug(slug: str) -> Dict[str, Any]:
    """Return a single taxonomy category by its slug.

    Raises :class:`KeyError` if the slug is not found.
    """
...
def get_category_by_number(number: int) -> Dict[str, Any]:
    ...
    Raises :class:`KeyError` if the number is not found.
    """
    for cat in PAIN_TAXONOMY:
...
def generate_faq_jsonld(category: Dict[str, Any]) -> Dict[str, Any]:
    """Generate JSON-LD FAQPage schema for a single pain category.

    Each panic search query becomes a question, and its legendary diagnostic
    becomes the answer.
    """
    main_entity = []
...
def generate_knowledge_graph_jsonld() -> Dict[str, Any]:
    """Generate a comprehensive JSON-LD Knowledge Graph for the entire site.

    Maps the 10 Pain Taxonomy categories to their legendary owners, CXO
    domains, and executable workflows.
    """
    items = []
    for cat in PAIN_TAXONOMY:
        items.append({
            ...
            "offers": {
                "@type": "Offer",
                "name": f"{cat['owner_legend']} Agent — {cat['owner_title']}",
                "description": (
                    f"The {cat['owner_legend']} agent ({cat['owner_title']}) "
                    f"addresses {cat['title'].lower()} through the "
                    f"{cat['workflow_id']} workflow."
                ),
                "url": chatroom_deep_link(
    ...
    return {
        ...
        "description": (
            "A comprehensive map of 10 organizational pain categories, each "
            "governed by a legendary CXO agent and backed by an executable "
            "workflow. Business Infinity meets founders at the exact moment "
            "their integrity is failing."
        ),
...
def generate_workflow_spec_jsonld(
    ...
    return {
        ...
        "name": f"{category['workflow_id']} — Business Infinity Workflow Spec",
        "description": category["disease"],
        "codeRepository": github_yaml_url(category["yaml_path"]),
...
def render_jsonld_script(data: Dict[str, Any]) -> str:
